...
namespace DataConveyer_HierarchicalJsonToCsv
{
   class Program
   {
      ...
      static void Main()
      {
         ...
         Console.WriteLine();
         Console.WriteLine("This application converts CSV data to/from hierarchical (unbound) JSON data.");
         Console.WriteLine("JSON nesting hierarchy is reflected in compound keys, i.e. CSV column names.");
         Console.WriteLine();
      ...
      private static async void FileDroppedHandler(object sender, FileSystemEventArgs e)
      {
         ...
         {
            Console.WriteLine($"Processing {fname} file completed in {stopWatch.Elapsed.TotalSeconds:##0.000}s; {result.ClustersWritten:N0} out of {result.ClustersRead:N0} records posted onto output.");
         }
